...
if __name__ == "__main__":
    ...
    print(df2)
    # region_aggregation = {}
    # region_aggregation['ABC'] = ['A', 'B', 'C']
    # region_aggregation['DE'] = ['D','E']
    ...
    # def create_aggregator(df, region_agg, industry_agg) -> array:
    # idx = df.index
    

    # def create_aggregator(df, names: array, indices: list, n: int, m: int, first_index = True) -> array:
    # '''
    # Creates index vectors to aggregate products/industries and regions in the MRIO tables
    ...
    #     n (int):            Length of first multi-index (regions by default)
    #     m (int):            Length of second multi-index (products/industries by default)
    ...
    # multi_index = pd.MultiIndex.from_product([regions, industries], names=['Region', 'Industry'])
